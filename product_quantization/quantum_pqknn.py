...
class QuantumProductQuantizationKNN:
    ...
    @staticmethod
    def _sign_aware_encode_rows(Xp: np.ndarray) -> np.ndarray:
        """
        Sign-aware positive/negative split per partition row:

            x -> [x_+, x_-]

        with x_+ = max(x, 0) and x_- = max(-x, 0).

        This makes x and -x distinguishable before the later
        amplitude-style normalization in the quantum distance / k-means path.
        """
        Xp = np.asarray(Xp, dtype=np.float64)
        X_pos = np.maximum(Xp, 0.0)
        X_neg = np.maximum(-Xp, 0.0)
        return np.concatenate([X_pos, X_neg], axis=1)

    @staticmethod
    def _sign_aware_encode_vector(xp: np.ndarray) -> np.ndarray:
        xp = np.asarray(xp, dtype=np.float64).reshape(-1)
        x_pos = np.maximum(xp, 0.0)
        x_neg = np.maximum(-xp, 0.0)
        return np.concatenate([x_pos, x_neg], axis=0)

    def _encode_partition_rows(self, Xp: np.ndarray) -> np.ndarray:
        Xp = np.asarray(Xp, dtype=np.float64)
        return self._sign_aware_encode_rows(Xp) if self.sign_aware_encoding else Xp

    def _encode_partition_vector(self, xp: np.ndarray) -> np.ndarray:
        xp = np.asarray(xp, dtype=np.float64).reshape(-1)
        return self._sign_aware_encode_vector(xp) if self.sign_aware_encoding else xp

    def _compress_partition(self, p_idx: int, Xp: np.ndarray):
        Xp = self._encode_partition_rows(Xp)
    ...
    def _partition_dists(self, sample: np.ndarray) -> np.ndarray:
        ...
        for p in range(self.n):
            ...
            s_part = self._encode_partition_vector(s_part)
